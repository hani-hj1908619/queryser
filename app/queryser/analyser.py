...
def get_simple_select_costs(
    ...
    for perm in clauses_perms:
        ...
        for clause in perm:
            ...
            col_type = repo.read_column_stats(
                table=table, column=clause.column
            ).column_type
            if isinstance(clause, EqualityFilter):
                if col_type == "number":
...
def get_join_select_costs_post(
    ...
    df1_select_cost, df1_after = get_simple_select_costs(
        df1_before, Table.EMPLOYEE, df1_filterrs
    )
    df2_select_cost, df2_after = get_simple_select_costs(
        df2_before, Table.TRADE_UNION, df2_filterrs
    )
...
def get_best_algorithms(query_info: QueryInfo) -> pd.DataFrame:
    ...
    if query_info.type == QueryType.NORMAL:
        ...
        for condition in query.where_attrs:
            ...
            index_types.append(col_stat.index_type or "none")
            is_unique.append(col_stat.is_unique)
